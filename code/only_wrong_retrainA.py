...
def retrain(filename,wrong=True,samplesize=100):
    ...
    if wrong==False:
        ...
        wrong_order_lst=[]         #排序前500里的错误用例
        for i in range(samplesize):
            if bound_lst[i] in notpasslist:
                wrong_order_lst.append(bound_lst[i])
        x_random=np.zeros((len(wrong_order_lst),28,28))
        y_random = np.zeros((len(wrong_order_lst),))
        
        for i in range(len(wrong_order_lst)):
            x_random[i] = x_test[wrong_order_lst[i]]
            y_random[i] = y_test[wrong_order_lst[i]]
        #print(wrong_order_lst)
...
length_of_wrong=0
notpass_num=0
datafilename_lst=['beta_mnist_shift_up3']
'''
        'beta_mnist_rotate(down2)',
...
for f in range(len(datafilename_lst)):
    ...
    lstramdon_acc=[]
    for i in range(5):
        testbound_acc,origin_acc,notpass_num = retrain(filename,wrong=False,samplesize=length_of_order)
        lstorder_acc.append(testbound_acc)
        print("random select............................")
        testramdon_acc, __,__ = retrain(filename,wrong =True,samplesize=length_of_order)
        lstramdon_acc.append(testramdon_acc)
        
#仅仅把排序前1%中用例中的错误用例拿出来训练        
    resultfilename = '../only_wrong_resultA.txt'
    with open(resultfilename,'a') as file_object:
        ...
        file_object.write('\n')
        file_object.write('notpass_num(samplesize:500): ')
        file_object.write(str(notpass_num))
        file_object.write('\n')
        ...
        file_object.write('\n')
        file_object.write('only_wrong_accuracy:\n')
